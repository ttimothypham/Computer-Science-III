...
public class ImageEnhancer extends Component implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if (e.getSource() != undoItem && e.getSource() != redoItem) //do the following if an action is not undo/redo 
    	{
    		undoStack.push(copyImage(biWorking)); //pushes the OG image into the undoStack
    		undoItem.setEnabled(!undoStack.isEmpty()); //enables the undo menu item if an action is avaliable to be undone
    		redoItem.setEnabled(false); //disables the redo menu item
    	}
    	...
    	{ 
    		if(undoStack.isEmpty()==false) //checks if theres something in the undo stack
        	{
        		BufferedImage undoImage = copyImage(undoStack.pop()); //pops the targeted image from the undo stack
        		redoStack.push(copyImage(biWorking)); //pushes the OG image into the redoStack
        		biFiltered = undoImage; //sets the current image to undoImage
        		undoItem.setEnabled(!undoStack.isEmpty()); //enables the undo menu if an action is available to be undone
        		redoItem.setEnabled(true); //enables the redo menu item
        		repaint();
        	}
    	}
    	if (e.getSource() == redoItem)
    	{
			if (!redoStack.isEmpty()) //checks if theres something in the redo stack
			{
				BufferedImage redoImage = copyImage(redoStack.pop()); //pops the targeted image from the redo stack
				undoStack.push(copyImage(biWorking)); //pushes the OG image into the undoStack
				biFiltered = redoImage; //sets the current image to redoImage
				undoItem.setEnabled(true); //enables the undo menu item
				redoItem.setEnabled(!redoStack.isEmpty()); //enables the redo menu item if an action is available to be redone
				repaint();
			}
		}
